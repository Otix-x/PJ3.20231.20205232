  ...
  return (
    <div>
      <p className="font-semibold">
        Dòng địa chỉ 1: <span className="font-normal">{line1}</span>
      </p>
      {line2 ? (
        <p className="font-semibold">
          Dòng địa chỉ 2: <span className="font-normal">{line2}</span>
        </p>
      ) : null}
      <div className="flex items-center space-x-2">
        <p className="font-semibold">
          Quận/Huyện: <span className="font-normal">{state}</span>
        </p>
        <p className="font-semibold">
          Tỉnh/Thành phố: <span className="font-normal">{city}</span>
        </p>
        <p className="font-semibold">
          Mã bưu điện: <span className="font-normal">{postal_code}</span>
        </p>
...
export default function OrderCard({ order, disableUpdate = true }: Props) {
  ...
  return (
    <div className="space-y-4 rounded bg-gray-200 p-2">
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className=" bg-orange-200">
          <tr>
            ...
            <th className="py-2 px-4 text-left">Tổng thanh toán</th>
          </tr>
        </thead>
        <tbody>
